...
export default class BookedCabScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    firebase.database().ref().child("bookings/" + this.getParamData.bokkingId).on('value', data => {
      ...
      firebase.database().ref().child("users/" + element.driver).on("value", (snapshot) => {
        const location = snapshot.val().location;

        if (!!location) {
          const destLoc = `${location.lat},${location.lng}`;

          const startLoc = `${element.pickup.lat},${element.pickup.lng}`;
          const route = `https://maps.googleapis.com/maps/api/directions/json?origin=${startLoc}&destination=${destLoc}&key=${google_map_key}&mode=driving`;
            ...
            .then((res) => {
              this.setState({
                driverLocation: {
                  ...
                  time: (res.routes[0].legs[0].duration['value'] / 60).toFixed(1)
                }
              })
            }
            );
    ...
    bookingResponse.on('value', currUserBookings => {
      ...
      if (currUserBookings.val()) {
        ...
        this.setState({
          ...
          customerName: currUserBookings.val().customer_name,
        }, () => {
